...
from source.dataloader import CERDatasetReader
...
os.makedirs(f'tensorboard/group_{group_id}', exist_ok=True)
os.makedirs(f'tensorboard/group_{group_id}/cluster_{cluster_id}', exist_ok=True)
os.makedirs(f'tensorboard/group_{group_id}/cluster_{cluster_id}/node_{target_node}')

os.makedirs('checkpoints', exist_ok=True)
os.makedirs('checkpoints/gcrnn', exist_ok=True)
os.makedirs(f'checkpoints/gcrnn/group_{group_id}', exist_ok=True)
os.makedirs(f'checkpoints/gcrnn/group_{group_id}/cluster_{cluster_id}', exist_ok=True)
os.makedirs(f'checkpoints/gcrnn/group_{group_id}/cluster_{cluster_id}/node_{target_node}', exist_ok=True)

os.makedirs('results', exist_ok=True)
os.makedirs(f'results/group_{group_id}/cluster_{cluster_id}', exist_ok=True)
os.makedirs(f'results/group_{group_id}/cluster_{cluster_id}/gcrnn', exist_ok=True)

#################################
# Setting up the datasets and data loaders
print('[*] Loading the data...')
train_dataset = CERDatasetReader(trains_x, trains_y, target_node, ToTensor())
train_loader = DataLoader(train_dataset, shuffle=True, batch_size=batch_size)

val_dataset = CERDatasetReader(vals_x, vals_y, target_node, ToTensor())
val_loader = DataLoader(val_dataset, shuffle=True, batch_size=batch_size)

test_dataset = CERDatasetReader(tests_x, tests_y, target_node, ToTensor())
...
summary = SummaryWriter(log_dir=f'tensorboard/group_{group_id}/cluster_{cluster_id}/node_{target_node}/')
#################################
...
for e in range(epochs):
    ...
    if avg_val_loss < best_val_loss:
        ...
        torch.save(model.state_dict(),
                   f'checkpoints/gcrnn/group_{group_id}/cluster_{cluster_id}/node_{target_node}/cer_model.pkl')

        with open(f'results/group_{group_id}/cluster_{cluster_id}/gcrnn/node_{target_node}.txt', 'w') as handle:
